## Closeout Protocol — Required at the End of Every Task
...
| 9 | Session has **unresolved pending items** (blocker, deferred work, multi-machine handoff) not capturable in a preference/pattern note? | Run Cleanup Protocol (delete resolved sessions), then write (once, condensed) | `sessions/YYYY-MM-DD-{slug}.md` in the **vault** |

**Required Closeout output** (in the final summary to the user):

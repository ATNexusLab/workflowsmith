...
when_to_use: >
  Use when designing a new REST, GraphQL, or gRPC API, reviewing an existing API for versioning, pagination, or error handling, or defining API contracts and producing OpenAPI or protobuf documentation.
---
...
Complete the `## Validation Checklist` before treating the contract as ready.
...
- Return HTTP 200 for errors — use the semantically correct 4xx/5xx status code
- Expose internal implementation details (stack traces, SQL errors, library versions) in error responses
- Design endpoints that return unbounded lists without pagination
- Break a versioned contract without incrementing the version
- Accept user-supplied data in URL paths without validation and sanitization
- Use verbs in REST resource URLs (`/getUser`) — resources are nouns, actions are HTTP methods
```

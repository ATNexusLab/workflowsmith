...
when_to_use: >
  Use when modeling a schema or data structure, creating or reviewing SQL or NoSQL migrations, or optimizing queries and choosing a database.
---
...
- Design a schema without considering indexes for the expected query patterns
- Use `TEXT` for every column when a constrained type (`VARCHAR`, `INTEGER`, `ENUM`) is more appropriate
- Skip migration rollback plans — every `UP` migration needs a `DOWN`
- Store passwords, tokens, or secrets in plain-text columns
- Use `SELECT *` in application queries — always project explicit columns
- Recommend destructive migrations (DROP COLUMN, DROP TABLE) without a data-backup step

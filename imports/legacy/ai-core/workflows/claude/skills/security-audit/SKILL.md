...
when_to_use: >
  Use when auditing code for OWASP-aligned vulnerabilities, reviewing dependencies for CVEs or insecure configuration, or producing a security report with severity classification.
---
...
When the user modifies `package.json`:
1. Run `npm audit --audit-level=high`
2. If high/critical findings exist → block merge, report each with CVE ID + fix version
3. If only low/medium → report but do not block

### Auth endpoint review
For a new login endpoint:
- Check: timing-safe password compare, rate limiting, user-enumeration-safe error messages
- Check: session token uses CSPRNG, HttpOnly+Secure+SameSite cookies
- Check: no stack trace or internal path in error responses

### IDOR check pattern
For any endpoint that takes a resource ID in the path or body:
- Verify server-side ownership check (not just "is the user logged in?")
- Test: authenticated user A requests resource owned by user B → must return 403
...
- Report vulnerabilities without severity classification (Critical/High/Medium/Low)
- Suggest "just disable the check" as a fix for a failing security control
- Mark a finding as false positive without documented evidence
- Recommend security-by-obscurity as a primary control
- Skip the dependency audit (`npm audit` / `pip-audit` / `cargo audit`) when the manifest was touched
- Generate exploit code or working attack payloads

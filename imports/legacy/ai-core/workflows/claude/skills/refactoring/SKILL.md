...
when_to_use: >
  Use when improving existing code without changing behavior, identifying and removing code smells, or extracting modules, simplifying functions, or reducing coupling.
---
...
- finish only when the `## Validation Checklist` is complete
...
- Change observable behavior while calling it a refactor — if behavior changes, it is a feature or fix
- Refactor without a test suite in place; if none exists, write characterization tests first
- Rename symbols across a large codebase in a single commit without incremental verification
- Mix refactoring commits with feature or bug-fix changes — keep them separate
- Remove code that appears unused without verifying it is not called dynamically or via reflection
- Apply a pattern (e.g., Strategy, Factory) before confirming the problem it solves is actually present

...
description: Use to create or update CLAUDE.md, SKILL.md, agent .md files, and path-specific rule files (.claude/rules/<topic>.md) with the correct scope, frontmatter, and usage rules.
when_to_use: >
  Use when creating or updating CLAUDE.md, SKILL.md, or agent .md files, or configuring path-specific rules (.claude/rules/<topic>.md with paths: frontmatter).
---
...
- Create or update a project-root `CLAUDE.md`
- Create or update a reusable `SKILL.md`
- Create or update a specialized agent `.md` file
- Configure file-path-specific rules with `.claude/rules/<topic>.md` and `paths:` frontmatter
- Audit instruction files for scope, frontmatter, duplication, or language drift

## Core Principle

Instruction files are reusable contract surfaces, not ad-hoc prompts. Keep durable framework guidance in skills, agent-specific execution stance in agent `.md` files, repository-wide policy in `CLAUDE.md`, and file-scoped conventions in `.claude/rules/<topic>.md` with `paths:` frontmatter.
...
## Instruction Surfaces in the Claude Code Ecosystem

| Type | File | Scope |
|------|------|-------|
| Global | `~/.claude/CLAUDE.md` | All user sessions in this environment |
| Repository | `CLAUDE.md` (project root) | The entire repository |
| Path-specific | `.claude/rules/<topic>.md` with `paths:` frontmatter | Matching files only |
| Skill | `skills/<name>/SKILL.md` | Reusable domain guidance loaded on demand |
| Agent | `agents/<name>.md` | Isolated agent context when the runtime supports it |

## Language and Scope Rules

- Follow the active repository instruction contract first.
- In this repository, all repository-owned instruction surfaces are written in **English**: `CLAUDE.md`, `.claude/rules/*.md`, `SKILL.md`, and agent `.md` files.
...
name: agent-name
description: >
  [Persona]. Use when [precise trigger].
model: sonnet
color: blue
tools: Read, Grep, Edit, Write, Bash, TodoRead, TodoWrite
---
```

Critical rules for agent files:
...
A agent file body should contain:
- Identity and role
...
# Rules for TypeScript test files
...
|----------|------------------|
| Repository-wide conventions | `CLAUDE.md` |
| Rules for a specific file class | `.claude/rules/<topic>.md` with `paths:` frontmatter |
| Reusable domain knowledge | `SKILL.md` |
| Persona with a complete workflow | Agent `.md` file |
...
- Affects the whole repository → `CLAUDE.md` (project root)
- Affects a specific file class or path → `.claude/rules/<topic>.md` with `paths:` frontmatter
- Defines reusable domain guidance → `SKILL.md`
- Defines a persona with execution stance → agent `.md` file
...
- Keep repo-wide policy in `CLAUDE.md`, not duplicated across every skill
- Keep path-specific rules in `.claude/rules/<topic>.md` with `paths:` frontmatter
...
### Minimal valid agent `.md` file

```markdown
---
name: my-agent
description: >
  Agent role. Use when you need [context].
model: sonnet
color: blue
tools: Read, Grep, Edit, Write, TodoRead, TodoWrite
---

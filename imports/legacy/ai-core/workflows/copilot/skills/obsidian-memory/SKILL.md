...
triggers:
  - "read from or write to the Obsidian vault"
  - "persist cross-project knowledge"
  - "recover context from previous sessions or projects"
  - "remember this, recall a past decision, or access cross-project memory"
license: MIT
---
...
Use this skill **after** the repository's local documentation check and lazy-memory gate have been considered. The gate should stay cheap:

- If local context and relevant repo docs are enough, **do not invoke** this skill
- If the task is on the local fast path, skip it entirely
- In a recurring project, read the vault only if the repository docs still leave a historical gap
- If a lazy trigger exists, invoke the skill and **stop as soon as context is sufficient**

**Do not read the vault by default.** Read only if at least one condition is true:

1. Local context and repo docs are **insufficient** for the task
2. The work is in a **recurring project** and the repo does not explain the needed rationale or continuity
3. A technical decision needs **history** from previous choices or learned patterns that are not documented in the repo
4. The user **explicitly asks** for memory ("remember what we did in X", "what was the decision about Y")

If none apply: **do not read**. Work from the available context and move on.

### Read Hierarchy (stop when sufficient)

Execute in order and stop as soon as enough context is available.

#### Step 1 — User preferences (when relevant to a decision)

Call `obsidian-get_note_content("knowledge/user-preferences.md")`.

Load it only when an upcoming decision could be guided by a prior preference.

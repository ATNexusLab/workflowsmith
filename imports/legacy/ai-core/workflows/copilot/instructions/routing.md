...
3. **Context Gate (lazy):** If the task may benefit from historical context, preferences, or cross-project patterns, invoke the `obsidian-memory` skill for project context, sessions, and patterns as needed. Skip when the fast path is sufficient.
4. Build the **skill stack** in this order: identify the global base skill(s) in `~/.copilot/skills`, then add local repo skill(s) and path-specific instructions that genuinely narrow the problem.
...
7. **Mandatory verbalization:** when the task is non-trivial (docs/memory/skill/routing decisions were actually considered), emit the decision trail in this fixed order:
   ...
   ```
   Skip in trivial fast-path reads, quick explanations, and tiny local edits. `global=` = skills from `~/.copilot/skills`; `local=` = repo-local skills. `Route:` = final owner, not every option considered. If this block does not appear on non-trivial tasks, the protocol was violated.
...
| Creating or updating `copilot-instructions.md`, `SKILL.md`, agent `.md` files | `copilot-instructions` | — |
